...
VIEW `vw_ace_character` AS
    ...
    FROM
        (((((((`ace_object` `ao`
        JOIN `ace_weenie_class` `awc` ON ((`ao`.`weenieClassId` = `awc`.`weenieClassId`)))
        JOIN `ace_object_properties_string` `aops` ON (((`ao`.`aceObjectId` = `aops`.`aceObjectId`)
            AND (`aops`.`strPropertyId` = 1))))
        JOIN `ace_object_properties_bool` `aopb` ON (((`ao`.`aceObjectId` = `aopb`.`aceObjectId`)
            AND (`aopb`.`boolPropertyId` = 9001))))
        JOIN `ace_object_properties_int` `aopi` ON (((`ao`.`aceObjectId` = `aopi`.`aceObjectId`)
            AND (`aopi`.`intPropertyId` = 1))))
        JOIN `ace_object_properties_bigint` `aopbi` ON (((`ao`.`aceObjectId` = `aopbi`.`aceObjectId`)
            AND (`aopbi`.`bigIntPropertyId` = 9001))))
        JOIN `ace_object_properties_iid` `aopiidacc` ON (((`ao`.`aceObjectId` = `aopiidacc`.`aceObjectId`)
            AND (`aopiidacc`.`iidPropertyId` = 9001))))
        LEFT JOIN `ace_object_properties_double` `aopd` ON (((`ao`.`aceObjectId` = `aopd`.`aceObjectId`)
