...
namespace ACE.Database
{
    public class Database
    {
        ...
        private void ConstructGetListStatement<T1>(T1 id, Type type)
        {
            ...
            DbTableAttribute dbTable = type.GetTypeInfo().GetCustomAttributes(false)?.OfType<DbTableAttribute>()?.FirstOrDefault();
            DbGetListAttribute getList = type.GetTypeInfo().GetCustomAttributes(false)?.OfType<DbGetListAttribute>()?.FirstOrDefault(d => d.ConstructedStatementId == statementId);
        ...
        public List<T2> ExecuteConstructedGetListStatement<T1, T2>(T1 id, Dictionary<string, object> criteria)
        {
            ...
            uint statementId = Convert.ToUInt32(id);
            DbGetListAttribute getList = typeof(T2).GetTypeInfo().GetCustomAttributes(false)?.OfType<DbGetListAttribute>()?.FirstOrDefault(d => d.ConstructedStatementId == statementId);
