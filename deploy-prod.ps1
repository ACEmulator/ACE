...
if (!$Force -and !$Emergency) {
    $confirm = Read-Host "⚠️ This will deploy to PRODUCTION. Are you sure? (yes/no)"
    if ($confirm -ne "yes") {
    ...
    try {
        ...
        if ($dbRunning) {
            ...
            if ($backups.Count -gt 10) {
                ...
                Write-Host "🗑️ Cleaned up old backups (keeping last 10)" -ForegroundColor Cyan
            }
        } else {
            Write-Host "⚠️ Production database not running!" -ForegroundColor Red
            if (!$Emergency) {
