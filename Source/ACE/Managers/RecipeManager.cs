...
namespace ACE.Managers
{
    public class RecipeManager
    {
        ...
        {
            // build the cache
            var recipeTask = Database.DatabaseManager.World.GetAllRecipes();
            recipeTask.Wait();
            var recipes = recipeTask.Result;
            _recipeCache = new RecipeCache(recipes);
        }

        public static async Task UseObjectOnTarget(Player player, WorldObject source, WorldObject target)
        {
            ...
            switch ((RecipeType)recipe.RecipeType)
            {
                case RecipeType.CreateItem:
                    await HandleCreateItemRecipe(player, source, target, recipe);
                    break;
                case RecipeType.Dyeing:
                    break;
                case RecipeType.Healing:
                    await HandleHealingRecipe(player, source, target, recipe);
                    break;
        ...
        private static async Task HandleCreateItemRecipe(Player player, WorldObject source, WorldObject target, Recipe recipe)
        {
            ...
            if (skillSuccess)
            {
                ...
                if ((recipe.ResultFlags & (uint)RecipeResult.SuccessItem1) > 0 && recipe.SuccessItem1Wcid != null)
                    newObject1 = await player.AddNewItemToInventory(recipe.SuccessItem1Wcid.Value);

                if ((recipe.ResultFlags & (uint)RecipeResult.SuccessItem2) > 0 && recipe.SuccessItem2Wcid != null)
                    newObject2 = await player.AddNewItemToInventory(recipe.SuccessItem2Wcid.Value);
            ...
            else
            {
                ...
                if ((recipe.ResultFlags & (uint)RecipeResult.FailureItem1) > 0 && recipe.FailureItem1Wcid != null)
                    newObject1 = await player.AddNewItemToInventory(recipe.FailureItem1Wcid.Value);

                if ((recipe.ResultFlags & (uint)RecipeResult.FailureItem2) > 0 && recipe.FailureItem2Wcid != null)
                    newObject2 = await player.AddNewItemToInventory(recipe.FailureItem2Wcid.Value);
        ...
        private static async Task HandleHealingRecipe(Player player, WorldObject source, WorldObject target, Recipe recipe)
        {
