...
namespace ACE.Entity
{
    public class Key : WorldObject
    {
        ...
        public async Task HandleActionUseOnTarget(Player player, WorldObject target)
        {
            if (player == null)
            {
                return;
                ...
                if (target.WeenieType == WeenieType.Door)
                {
                    ...
                    switch (results)
                    {
                        case Entity.Door.UnlockDoorResults.UnlockSuccess:
                            ...
                            if (Structure < 1)
                            {
                                await player.RemoveItemFromInventory(Guid.Full, player.Guid.Full, 1);
                            }
