...
namespace ACE.Factories
{
    public class CorpseObjectFactory
    {
        ...
        {
            const ushort wcidCorpse = 21;
            const WeenieHeaderFlag weenie = WeenieHeaderFlag.ItemCapacity | WeenieHeaderFlag.ContainerCapacity | WeenieHeaderFlag.Usable | WeenieHeaderFlag.UseRadius | WeenieHeaderFlag.Burden;
            const ObjectDescriptionFlag objDesc = ObjectDescriptionFlag.CanOpen | ObjectDescriptionFlag.Stuck | ObjectDescriptionFlag.Attackable | ObjectDescriptionFlag.Corpse; // = bitfield 8213
            var name = $"Corpse of {template.Name}";
            var wo = new Container(ObjectType.Container,
                new ObjectGuid(CommonObjectFactory.DynamicObjectId, GuidType.None), name, wcidCorpse, objDesc, weenie,
                newPosition)
            {
                CurrentMotionState = new UniversalMotion(MotionStance.Standing, new MotionItem(MotionCommand.Dead)),
                MTableResourceId = template.MTableResourceId,
                Stable = 536871106,
                CSetup = template.CSetup,
                Petable = 872415342,
                ObjScale = template.ObjScale,
                PhysicsDescriptionFlag = PhysicsDescriptionFlag.CSetup | PhysicsDescriptionFlag.MTable |
                                         PhysicsDescriptionFlag.ObjScale | PhysicsDescriptionFlag.Stable |
                                         PhysicsDescriptionFlag.Petable | PhysicsDescriptionFlag.Position |
                                         PhysicsDescriptionFlag.Movement,
                PhysicsState = PhysicsState.Ethereal | PhysicsState.IgnoreCollision | PhysicsState.Gravity
            };
            ...
            // MotionTableId in db
            // SoundTableId in DB - constant value according to pcap
            // ModelTableId in DB
            // phstableid in DB - constant value according to pcap

            // 104579 - according to pcap
            // 1044 - according to pcap
