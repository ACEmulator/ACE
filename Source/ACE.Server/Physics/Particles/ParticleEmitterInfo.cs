...
namespace ACE.Server.Physics
{
    public class ParticleEmitterInfo
    {
        ...
        {
            var result = ACE.Common.ThreadSafeRandom.Next(-1.0f, 1.0f) * ScaleRand + StartScale;
        ...
        {
            var result = ACE.Common.ThreadSafeRandom.Next(-1.0f, 1.0f) * ScaleRand * FinalScale;
            result = result.Clamp(0.1f, 10.0f);
        ...
        {
            var result = ACE.Common.ThreadSafeRandom.Next(-1.0f, 1.0f) * TransRand * StartTrans;
        ...
        {
            var result = ACE.Common.ThreadSafeRandom.Next(-1.0f, 1.0f) * TransRand * FinalTrans;
            result = result.Clamp(0.0f, 1.0f);
        ...
        {
            var result = ACE.Common.ThreadSafeRandom.Next(-1.0f, 1.0f) * LifeSpanRand + LifeSpan;
        ...
        {
            var rng = new Vector3(
                ACE.Common.ThreadSafeRandom.Next(-1.0f, 1.0f),
                ACE.Common.ThreadSafeRandom.Next(-1.0f, 1.0f),
                ACE.Common.ThreadSafeRandom.Next(-1.0f, 1.0f));
            ...
            var scaled = randomAngle * ((MaxOffset - MinOffset) + MinOffset) * ACE.Common.ThreadSafeRandom.Next(0.0f, 1.0f);
            return scaled;
        }

        public Vector3 GetRandomA()
        {
            var rng = ACE.Common.ThreadSafeRandom.Next(0.0f, 1.0f);
