    ...
    public static class HouseManager
    {
        ...
        public static void HandleEviction(House house, uint playerGuid, bool multihouse = false, bool force = false)
        {
            ...
            if (!PropertyManager.GetBool("house_rent_enabled", true).Item && !multihouse && !force)
            {
                ...
                log.Debug($"HouseManager.HandleRentPaid({player.Name}): house rent disabled via config");
        ...
        private static bool IsRentPaid(PlayerHouse playerHouse)
        {
            ...
            foreach (var rentItem in houseData.Rent)
            {
                ...
                {
                    log.Debug($"{playerHouse.PlayerName}.IsRentPaid() - required wcid {rentItem.WeenieID} amount {rentItem.Num:N0}, found {rentItem.Paid:N0}");
                    return false;
                }
            }
            return true;
        ...
        private static bool HasRequirements(PlayerHouse playerHouse)
        {
            ...
            if (player == null)
            {
                log.Warn($"{playerHouse.PlayerName}.HasRequirements() - couldn't find player");
                return false;
            }
            ...
            {
                log.Debug($"{playerHouse.PlayerName}.HasRequirements() - allegiance rank {rank} < {allegianceMinLevel}");
                return false;
            }
            return true;
        }
