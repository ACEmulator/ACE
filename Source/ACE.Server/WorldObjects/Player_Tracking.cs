...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
        ...
        public bool RemoveTrackedObject(WorldObject worldObject, bool fromPickup)
        {
            ...
            if (fromPickup && (!worldObject.WielderId.HasValue || (worldObject.ParentLocation ?? 0) == ACE.Entity.Enum.ParentLocation.None))
                Session.Network.EnqueueSend(new GameMessagePickupEvent(worldObject));
            else if (fromPickup && worldObject.WielderId.HasValue && worldObject.ParentLocation.HasValue)
                Session.Network.EnqueueSend(new GameMessagePickupEvent(worldObject), new GameMessageParentEvent(worldObject.Wielder, worldObject, (int)worldObject.ParentLocation, (int)(worldObject.Placement ?? 0)));
            else
