    ...
    partial class WorldObject
    {
        ...
        public void NotifyOfEvent(RegenerationType regenerationType)
        {
            ...
            //{
                ...
                {
                    if ((!(Generator is Container) || Generator.GeneratorAutomaticDestruction) && Generator.InitCreate > 0 && (Generator.CurrentCreate + removeQueueTotal - removeQueueTotal) == 0) // Parent generator is non-container (Container, Corpse, Chest, Slumlord, Storage, Hook, Creature) generator
                        Generator.Destroy(); // Generator's complete spawn count has been wiped out
                }
                else if (Generator.GeneratorAutomaticDestruction && Generator.InitCreate > 0 && (Generator.CurrentCreate + removeQueueTotal - removeQueueTotal) == 0)
                {
                    Generator.Destroy(); // Generator's complete spawn count has been wiped out
                }
            //}
