    ...
    partial class Player
    {
        ...
        public float DamageTarget(Creature target, WorldObject damageSource)
        {
            ...
            if (damage > 0.0f && target.Health.Current > 0)
            {
                ...
                {
                    var painSound = (Sound)Enum.Parse(typeof(Sound), "Wound" + Common.ThreadSafeRandom.Next(1, 3), true);
        ...
        public override void OnEvade(WorldObject attacker, AttackType attackType)
        {
            ...
            if (CombatMode != CombatMode.NonCombat)
            {
                if (defenseSkill.AdvancementClass >= SkillAdvancementClass.Trained)
                {
                    ...
                    if (noStaminaUseChance < Common.ThreadSafeRandom.Next(0.0f, 1.0f))
        ...
        public float CalculateDamage(WorldObject target, WorldObject damageSource, ref bool criticalHit, ref bool sneakAttack)
        {
            ...
            var baseDamage = Common.ThreadSafeRandom.Next(baseDamageRange.Min, baseDamageRange.Max);
