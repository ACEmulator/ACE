    ...
    partial class Player
    {
        ...
        public void SendMotionAsCommands(MotionCommand motionCommand)
        {
            if (FastTick)
            {
                ...
                EnqueueMotionAction(actionChain, new List<MotionCommand>() { motionCommand }, 1.0f, CurrentMotionState.Stance);
                actionChain.EnqueueChain();
            }
            else
            {
                //var motion = new Motion(MotionStance.NonCombat, MotionCommand.Ready);
                var motion = new Motion(CurrentMotionState.Stance, MotionCommand.Ready);
