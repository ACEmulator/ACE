...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
        ...
        public void EquipItemFromSet(WorldObject item)
        {
            ...
            var spells = GetSpellSet((EquipmentSet)item.EquipmentSetId, item.ItemXpStyle ?? 0, setItems);

            // get the spells from before / without this item
            setItems.Remove(item);
            var prevSpells = GetSpellSet((EquipmentSet)item.EquipmentSetId, item.ItemXpStyle ?? 0, setItems);
        ...
        public void OnItemLevelUp(WorldObject item, int prevItemLevel)
        {
            ...
            var prevSpells = GetSpellSet((EquipmentSet)item.EquipmentSetId, item.ItemXpStyle ?? 0, setItems, levelDiff);
        ...
        public void AuditItemSpells()
        {
            ...
            foreach (var enchantment in enchantments)
            {
                ...
                var currentSpells = GetSpellSet((EquipmentSet)item.EquipmentSetId, item.ItemXpStyle ?? 0, setItems);
