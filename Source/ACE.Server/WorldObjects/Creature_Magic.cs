...
namespace ACE.Server.WorldObjects
{
    partial class Creature
    {
        ...
        public void RemoveItemSpell(WorldObject item, uint spellId, bool silent = false)
        {
            ...
            {
                if (!silent)
                    target.EnchantmentManager.Remove(target.EnchantmentManager.GetEnchantment(spellId));
                else
                    target.EnchantmentManager.Dispel(target.EnchantmentManager.GetEnchantment(spellId));
            }
        }
    }
}
