    ...
    public static class AdminShardCommands
    {
        ...
        public static void ShutdownServer(Session session, params string[] parameters)
        {
            ...
            foreach (var player in PlayerManager.GetAllOnline())
            {
                ...
                player.Session.EnqueueSend(new GameMessageSystemChat(shutdownText + "\n" + timeRemaining, ChatMessageType.WorldBroadcast));

                if (adminShutdownText.Length > 0)
                    player.Session.EnqueueSend(new GameMessageSystemChat($"Message from {shutdownInitiator}: {adminShutdownText}", ChatMessageType.WorldBroadcast));
            }
