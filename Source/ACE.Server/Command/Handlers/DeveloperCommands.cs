        ...
        */
        ...
        public static void HandleForceLogoff(Session session, params string[] parameters)
        {
            ...
            if (target != null && target is Player player)
            {
                ...
                msg += $"------- Session: {(player.Session != null ? $"{player.Session.EndPointC2S}" : "NULL")}\n";
        ...
        public static void HandleShowSession(Session session, params string[] parameters)
        {
            ...
            {
                if (player.Session != null)
                    session.Network.EnqueueSend(new GameMessageSystemChat($"Session IP: {player.Session.EndPointC2S} | ClientId: {player.Session.Network.ClientId} is connected to Character: {player.Name} (0x{player.Guid.Full.ToString("X8")}), Account: {player.Account.AccountName} ({player.Account.AccountId})", ChatMessageType.Broadcast));
                else
