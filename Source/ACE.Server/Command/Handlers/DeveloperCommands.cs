        ...
        public static void HandleVendorDump(Session session, params string[] parameters)
        {
            ...
            if (session.Player.HealthQueryTarget.HasValue || session.Player.ManaQueryTarget.HasValue || session.Player.CurrentAppraisalTarget.HasValue)
            {
                ...
                if (wo is Vendor vendor)
                {
                    ...
                    {
                        var errorMsg = $"WCID {currencyWCID}{(vendor.AlternateCurrency.HasValue ? ", which comes from PropertyDataId.AlternateCurrency," : "")} is not found in the database, which will result in a server crash when the vendor is interacted with!\n";
                        msg += errorMsg;
                        log.Error($"[VENDOR] {wo.Name} (0x{wo.Guid}:{wo.WeenieClassId}) Currency {errorMsg}");
                    }
                    ...
                    foreach (var shopItem in createListShop)
                    {
                        ...
                        else
                        {
                            ...
                            var value = itemWeenie.GetProperty(PropertyInt.Value) ?? 1;
                            var sellRate = vendor.SellPrice ?? 1.0;
                            var itemType = itemWeenie.GetProperty(PropertyInt.ItemType) ?? 0;
                            if ((ItemType)itemType == ItemType.PromissoryNote)
                                sellRate = 1.15;
                            var cost = Math.Max(1, (uint)Math.Ceiling(((float)sellRate * value) - 0.1));
                    ...
                    foreach (var shopItem in vendor.UniqueItemsForSale.Values)
                    {
                        ...
                        var buyRate = vendor.BuyPrice ?? 1;
                        if (shopItem.ItemType == ItemType.PromissoryNote)
                            buyRate = 1.0;
                        var payout = Math.Max(1, (int)Math.Floor(((float)buyRate * (shopItem.Value ?? 0)) + 0.1));
                        msg += $"Paid: {payout:N0} {(payout == 1 ? currencyWeenie.GetName() : currencyWeenie.GetPluralName())}\n";

                        var value = shopItem.Value ?? 1;
                        var sellRate = vendor.SellPrice ?? 1.0;
                        var itemType = shopItem.ItemType;
                        if (itemType == ItemType.PromissoryNote)
                            sellRate = 1.15;
                        var cost = Math.Max(1, (uint)Math.Ceiling(((float)sellRate * shopItem.Value ?? 0) - 0.1));
                        msg += $"Cost: {cost:N0} {(cost == 1 ? currencyWeenie.GetName() : currencyWeenie.GetPluralName())}\n";
