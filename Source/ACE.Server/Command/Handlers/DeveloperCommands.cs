...
namespace ACE.Server.Command.Handlers
{
    public static class DeveloperCommands
    {
        ...
        */
        ...
        {
            Console.WriteLine($"\nKnown players to {session.Player.Name}: {session.Player.PhysicsObj.ObjMaint.ObjectTable.Values.Count(o => o.IsPlayer)}");
        ...
        {
            Console.WriteLine($"\nVisible players to {session.Player.Name}: {session.Player.PhysicsObj.ObjMaint.VisibleObjectTable.Values.Count(o => o.IsPlayer)}");
        ...
        {
            Console.WriteLine($"\nVoyeurs for {session.Player.Name}: {session.Player.PhysicsObj.ObjMaint.VoyeurTable.Values.Count(o => o.IsPlayer)}");
        ...
        public static void HandleLootGen(Session session, params string[] parameters)
        {
            ...
            if (!wcid)
            {
                session.EnqueueSend(new GameMessageSystemChat($"WCID must be a valid weenie id", ChatMessageType.Broadcast));
                return;
            }
            ...
            if (parameters.Length > 1)
            {
                ...
                {
                    session.EnqueueSend(new GameMessageSystemChat($"Loot Tier must be number between 1 and 8", ChatMessageType.Broadcast));
                    return;
                }
            }
            ...
            if (lootTest == null)
            {
                session.EnqueueSend(new GameMessageSystemChat($"{weenieClassId} is not a valid wcid.", ChatMessageType.Broadcast));
                return;
            }

            if (lootTest.TsysMutationData == null)
            {
                session.EnqueueSend(new GameMessageSystemChat($"Weenie has a missing PropertyInt.TsysMutationData needed for this function.", ChatMessageType.Broadcast));
                return;
            }
