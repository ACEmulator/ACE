...
namespace ACE.Server.Command.Handlers
{
    public static class PlayerCommands
    {
        ...
        {
            session.EnqueueSend(new GameMessageSystemChat($"Current world population: {PlayerManager.GetAllOnline().Count.ToString()}\n", ChatMessageType.Broadcast));
        }
        ...
        public static void HandleQuests(Session session, params string[] parameters)
        {
            ...
            foreach (var playerQuest in session.Player.QuestManager.Quests)
            {
                ...
                session.EnqueueSend(new GameMessageSystemChat(text, ChatMessageType.Broadcast));
            }
        }
    }
}
