        ...
        public static void HandleFinger(Session session, params string[] parameters)
        {
            ...
            else
            {
                ...
                if (account != null)
                {
                    ...
                    message += $"Account created on {account.CreateTime.ToLocalTime()} by IP: {(account.CreateIP != null ? new IPAddress(account.CreateIP) : null)} \n";
                    message += $"Account last logged on at {account.LastLoginTime?.ToLocalTime()} by IP: {(account.LastLoginIP != null ? new IPAddress(account.LastLoginIP) : null)}\n";
