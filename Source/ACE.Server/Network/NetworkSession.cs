    ...
    public class NetworkSession
    {
        ...
        private void SendPacketRaw(ServerPacket packet)
        {
            ...
            try
            {
                ...
                var socket = (session.State < SessionState.AuthConnected || session.State == SessionState.TerminationStarted) ? connectionC2S.Socket : connectionS2C.Socket;
                var endPoint = (session.State < SessionState.AuthConnected || session.State == SessionState.TerminationStarted) ? session.EndPointC2S : session.EndPointS2C ?? session.EndPointC2S;
