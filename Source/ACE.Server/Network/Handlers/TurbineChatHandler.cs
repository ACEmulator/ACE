            ...
            if (chatBlobType == ChatNetworkBlobType.NETBLOB_REQUEST_BINARY)
            {
                ...
                if (allegiance != null)
                {
                    ...
                    foreach (var member in allegiance.Members.Keys)
                    {
                        ...
                        if (allegiance.IsFiltered(member) || online.SquelchManager.Squelches.Contains(session.Player)) continue;
                ...
                else
                {
                    foreach (var recipient in PlayerManager.GetAllOnline())
                    {
                        ...
                        if (recipient.SquelchManager.Squelches.Contains(session.Player))
                            continue;
