    ...
    public static class AuthenticationHandler
    {
        ...
        private static void DoLogin(Session session, PacketInboundLoginRequest loginRequest)
        {
            ...
            catch (Exception ex)
            {
                ...
                session.BootSession(BootReason.AccountSelectCallbackException);
            }
        }
        ...
        private static void AccountSelectCallback(Account account, Session session, PacketInboundLoginRequest loginRequest)
        {
            ...
            if (session.Network.ConnectionData.ServerSeed == null || session.Network.ConnectionData.ClientSeed == null)
            {
                ...
                session.BootSession(BootReason.BadHandshake, new GameMessageCharacterError(CharacterError.Undefined));
                return;
            }
            ...
            if (loginRequest.NetAuthType < NetAuthType.AccountPassword)
            {
                if (loginRequest.Account == "acservertracker:jj9h26hcsggc")
                {
                    ...
                    session.BootSession(BootReason.PongSentClosingConnection, new GameMessageCharacterError(CharacterError.Undefined));
                ...
                session.BootSession(BootReason.NotAuthorizedNoPasswordOrGlsTicketIncludedInLoginReq, new GameMessageCharacterError(CharacterError.AccountInUse));
            ...
            if (account == null)
            {
                session.BootSession(BootReason.NotAuthorizedAccountNotFound, new GameMessageCharacterError(CharacterError.AccountDoesntExist));
                return;
            }
            ...
            if (WorldManager.Find(account.AccountName) != null)
            {
                ...
                session.BootSession(BootReason.AccountInUse, new GameMessageCharacterError(CharacterError.AccountInUse));
                return;
            }
            ...
            if (loginRequest.NetAuthType == NetAuthType.AccountPassword)
            {
                if (!account.PasswordMatches(loginRequest.Password))
                {
                    ...
                    session.BootSession(BootReason.NotAuthorizedPasswordMismatch, new GameMessageCharacterError(CharacterError.AccountInUse));
            ...
            else if (loginRequest.NetAuthType == NetAuthType.GlsTicket)
            {
                ...
                session.SendCharacterError(CharacterError.AccountInUse);
                session.BootSession(BootReason.NotAuthorizedGlsTicketNotImplementedToProcLoginReq, new GameMessageCharacterError(CharacterError.AccountInUse));
