    ...
    partial class Program
    {
        private static void DoOutOfBoxSetup(string configFile)
        {
            ...
            if (!variable.Equals("n", StringComparison.OrdinalIgnoreCase) && !variable.Equals("no", StringComparison.OrdinalIgnoreCase))
            {
                ...
                PatchDatabase("PKKills", config.MySql.PKKills.Host, config.MySql.PKKills.Port, config.MySql.PKKills.Username, config.MySql.PKKills.Password, config.MySql.PKKills.Database, config.MySql.Shard.Database, config.MySql.World.Database);
            }
