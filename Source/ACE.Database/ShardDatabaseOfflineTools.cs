...
namespace ACE.Database
{
    public static class ShardDatabaseOfflineTools
    {
        ...
        public static void FixSpellBarsPR2918(out int numberOfRecordsFixed)
        {
            ...
            log.Info($"Starting FixSpellBarsPR2918 process. This could take a while...{(debug ? "": " There will be no output while the fixes are applied.")}");
            ...
            using (var context = new ShardDbContext())
            {
                ...
                foreach (var character in characterIds)
                {
                    for (var spellBarNumber = 1; spellBarNumber < 9; spellBarNumber++)
                    {
                        ...
                        //var characterSpellBars = context.CharacterPropertiesSpellBar.Where(c => c.CharacterId == character && c.SpellBarNumber == spellBarNumber).ToList();
